...
import { ArrowLeft, BookOpen, Globe, Users, Edit3, BarChart2, Wand2, Plus, Save, Layers, UserPlus, StopCircle, Trash2, Library, Sparkles, Feather, Compass, GitMerge, FileText, Activity, Link, Star, PlayCircle } from 'lucide-vue-next';
  ...
  try {
    for (let i = 0; i < autoWriteCount.value; i++) {
      ...
      if (lastChapter && lastChapter.status !== 'completed') {
         if (lastChapter.content.trim().length > 0 && !isScoreAcceptable(getChapterReviewScore(lastChapter.reviewFeedback))) {
            activeChapterId.value = lastChapter.id;
            selectChapter(lastChapter.id);
         } else if (lastChapter.content.trim().length > 0) {
      ...
      while (attempts < maxAttempts && isAutoWriting.value) {
        ...
        // After revising, we MUST do one final review to get the score of the newly revised content
        // otherwise the chapter will be marked completed but the feedback & score won't match the new text!
        if (isAutoWriting.value) {
          autoWriteProgress.value = `正在进行重写后复核...`;
          await agentReviewChapter(true);
        }
...
const generateNextContent = async (silent = false) => {
...
3. 写作内容时绝对不要使用破折号（——），这会造成阅读的割裂感。请使用其他标点符号（如逗号、句号或省略号）来代替转折或语气停顿。
`;
    ...
    if (!silent) alert('续写失败: ' + e.message);
    else throw e;
  } finally {
    generating.value = false;
  }
              ...
              <div v-for="ch in chapters" :key="ch.id" @click="selectChapter(ch.id)"
                ...
                <div class="chapter-item-header">
                  ...
                  <div style="display: flex; align-items: center; gap: 6px;">
                    <span v-if="ch.status === 'completed' && getChapterReviewScore(ch.reviewFeedback)" 
                    ...
                    <button @click.stop="deleteChapter(ch.id, $event)" class="btn-icon" style="padding: 2px; color: var(--color-text-muted);" title="删除章节">
                      <Trash2 class="icon-sm" />
                    </button>
                  </div>
                </div>
          ...
          <div v-if="activeChapterId" class="workspace-inner">
            ...
            <div class="toolbar">
                <div class="toolbar-group">
                  <el-button v-if="!generating && !isAutoWriting" @click="showAutoWriteModal = true" size="small" type="success" title="全自动连载：生成->审核->重写->定稿">
                  ...
                  <el-button v-if="!generating && !isAutoWriting" @click="() => generateNextContent(false)" :disabled="currentChapter?.status === 'completed'" size="small">
