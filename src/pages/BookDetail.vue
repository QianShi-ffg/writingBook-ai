...
const generateOutline = async () => {
  ...
  try {
    ...
    // 生成完成后提示是否进行衍生设定
    confirmAction('大纲生成完成！是否根据当前大纲自动生成世界观设定、人物卡片及境界体系？\n\n【确认】自动衍生设定\n【取消】稍后手动生成', async () => {
      await autoGenerateCascade();
    });
  } catch (e: any) {
...
${book.value?.outline}`;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (parts.length >= 4 && parts[0] !== '起始章序号') {
        nodes.push({
          id: `node-${Date.now()}-${i}`,
  ...
  confirmAction(confirmMsg, async () => {
    ...
    try {
      if (hasExistingData) {
        ...
        await store.updateBook(bookId.value, { worldview: '' });
        for (const r of realms.value) await store.deleteRealm(r.id);
        for (const c of characters.value) await store.deleteCharacter(c.id);
      }

      // 1. Generate Worldview
      cascadeProgress.value = '正在分析大纲并生成世界观与背景设定...';
      ...
      // 2. Generate Realms if type is 修炼
...
世界观：${wvResult}`;
        ...
        try {
          ...
          for (let i = 0; i < lines.length; i++) {
            ...
            if (name && name !== '境界名称') {
              await store.createRealm({ bookId: bookId.value, level: i + 1, name, description: desc || '' });
            }
          }
        } catch (e) {
      ...
      cascadeProgress.value = '正在提取核心角色并生成人物卡片...';
...
const reviseChapter = async () => {
  ...
  confirmAction('这将会根据右侧的主编审核意见，完全重写并覆盖当前章节的正文。确认要继续吗？', async () => {
    generating.value = true;
    try {
      const activeChars = characters.value.filter(c => c.status === '出场').map(c => `${c.name}(${c.role}): ${c.personality}, 能力:${c.abilities}`).join('\n');
      const realmsInfo = book.value?.type === '修炼' ? realms.value.map(r => `${r.level}.${r.name}:${r.description}`).join('\n') : '无';
  ...
  <div class="dashboard-layout">
    ...
    <header class="app-header">
      <div class="header-left">
        ...
        <span v-if="book" class="badge badge-info">{{ book.type }}</span>
      </div>
    ...
    <main class="main-content">
      ...
      <!-- Outline Tree Tab -->
      <div v-if="activeTab === 'outline-tree'" class="tab-pane scrollable">
          ...
          <div v-if="activeChapterId" class="workspace-inner">
            ...
            <div class="editor-split">
              ...
              <div class="editor-wrapper" :class="{ 'scanning': currentChapter?.status === 'reviewing' && generating }">
                <textarea ref="mainEditorRef" v-model="currentChapterContent" @input="saveChapter" class="main-editor" placeholder="开始写作..."></textarea>
              ...
              <aside class="summary-panel">
                ...
                <div style="flex: 1; display: flex; flex-direction: column; overflow: hidden;">
                  ...
                  <div v-if="currentChapter?.status === 'reviewing' || currentChapterFeedback" class="feedback-section" style="flex: 1; display: flex; flex-direction: column; padding: var(--spacing-md); border-bottom: 1px solid var(--color-border);">
                    ...
                    <textarea v-model="currentChapterFeedback" @input="saveChapter" class="summary-editor" style="flex: 1; width: 100%; min-height: 200px; background: rgba(37, 99, 235, 0.05); border-radius: 4px; padding: 8px;" placeholder="AI主编审核意见将显示在这里..."></textarea>
                  </div>
                  ...
                  <div class="summary-section" style="flex: 1; overflow-y: auto; padding: var(--spacing-md); display: flex; flex-direction: column; position: relative;">
                    ...
                    <textarea v-model="currentChapterSummary" @input="saveChapter" class="summary-editor" style="flex: 1; width: 100%;" placeholder="点击上方【AI梳理总结】提取本章剧情点和人物动向，方便生成下一章时作为上下文参考..."></textarea>
